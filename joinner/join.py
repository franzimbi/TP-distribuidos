...
class Join:
    def __init__(self, join_queue, column_id, column_name, use_diskcache=False):
        ...
        logging.info("action: receive_join_data | status: finished | entries: %d | diskcache: {%s}", len(self.join_dictionary), str(use_diskcache))
        # self.query4 = False
    ...
    def callback(self, ch, method, properties, message):
        ...
        try:
            ...
            logging.error(f'action: join_batch_with_dicctionary | result: fail | errror: {e} | dic: {self.join_dictionary}')
